...
const proxy_hook = new Webhook(WEBHOOK_URL, { proxy: PROXY });

const IMAGE_URL = 'https://homepages.cae.wisc.edu/~ece533/images/airplane.png';

hook.setUsername('Discord Webhook Node Name');
hook.setAvatar(IMAGE_URL);
proxy_hook.setUsername('Discord Webhook Node Name');
proxy_hook.setAvatar(IMAGE_URL);
...
describe('Proxy hooks', function(){
    it('Sends embed via a proxy', function(done){
        const embed = new MessageBuilder();
        ...
        proxy_hook.send(embed).then(() => {
            done();
        })
        .catch(err => done(err));
    });

    it('Sends text to webhook via a proxy', function(done){
        proxy_hook.send('Plain text').then(() => {        
            done();
        })
        .catch(err => done(err));
    });
